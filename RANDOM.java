...
public class RANDOM extends Lab4 {
...
//							cp.loadtime = time;
						...
						//-------------NO FREE FRAMES --> USE REPLACEMENT ALGORITHM	
							...
							currentf = random(framet, random);
							System.out.println("Fault, using random");
	...
	public Frame random(ArrayList <Frame> framet, Scanner r) {
		//this is the replacement algorithm Random
		//we want to evict a random frame
		Frame starter = framet.get(0);//start from the beginning
		
		int randomframeid = r.nextInt() % framet.size();//make sure that the random frame is within the total number of frames
		for(int g = 0; g < framet.size(); g++) {
			//keep comparing the current starting frame with all other frames in the frame table
			//to see which one's matches the randome frame id
			if(framet.get(g).frameID == randomframeid) {
